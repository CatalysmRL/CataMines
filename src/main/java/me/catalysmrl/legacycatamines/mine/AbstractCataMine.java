package me.catalysmrl.legacycatamines.mine;
...
import me.catalysmrl.legacycatamines.mine.components.CataMineType;
...
public abstract class AbstractCataMine implements Cloneable {
    ...
    protected CataMineExecutor executor = new CataMineExecutor(this);
    ...
    public boolean checkRunnable() {
        if (region == null) {
            ConfigFile configFile = new ConfigFile(CataMines.getInstance().getDataFolder() + "/mines", name + ".yml");
            if (Bukkit.getWorld(world) != null) {
                loadRegion(configFile);
            } else {
                setStopped(true);
    ...
    public void loadRegion(ConfigFile configFile) {

        setStopped(true);
        CataMines.getInstance().getLogger().severe("Region recovery is no longer supported since v3.0 (" + name + ")\n" +
                "Stopped mine to prevent console spam");
    }

    public void addBlock(CataMineBlock cataMineBlock) {
        double chanceSum = 0;
        for (CataMineBlock block : blocks) {
            if (block.getBlockState().equals(cataMineBlock.getBlockState())) {
                continue;
            }
        ...
        for (int i = 0; i < blocks.size(); i++) {
            if (blocks.get(i).getBlockState().equals(cataMineBlock.getBlockState())) {
    ...
    public double getBlockChance(Material material) {
        ...
        for (CataMineBlock block : blocks) {
            if (block.getBlockState().getMaterial().equals(material)) {
    ...
    public CataMineBlock getBlock(BlockData blockData) {
        for (CataMineBlock block : blocks) {
            if (block.getBlockState().equals(blockData)) {
                return block;
            }
        }
    ...
    public boolean containsBlockData(BlockData blockData) {
        for (CataMineBlock block : blocks) {
            if (block.getBlockState().equals(blockData)) return true;
        }
    ...
    public boolean containsBlockMaterial(Material material) {
        for (CataMineBlock block : blocks) {
            if (block.getBlockState().getMaterial().equals(material)) return true;
        }
    ...
    public void blocksToRandomPattern() {
        ...
        blocks.stream().filter(cataMineBlock -> cataMineBlock.getChance() > 0)
                .forEach(cataMineBlock -> randomPattern.add(BukkitAdapter.adapt(cataMineBlock.getBlockState()).toBaseBlock(), cataMineBlock.getChance()));
    }

    public CataMineExecutor getExecutor() {
        return executor;
    }

    public void setExecutor(CataMineExecutor executor) {
        this.executor = executor;
    }

    public String getTranslatedTimeMessage() {
        Duration duration = Duration.ofSeconds(countdown);
        return duration.toString();
    }

    public void broadcastHotbar() {
        String finalMessage = Utils.setPlaceholders(getWarnHotbarMessage(resetMode), this);
        getPlayersInDistance().forEach(player -> player.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(finalMessage)));
    }

    public void broadcastWarnMessage() {
        String finalWarnMessage = Utils.setPlaceholders(getWarnMessage(), this);
        getPlayersToWarn().forEach(player -> Arrays.stream(finalWarnMessage.split("/n")).forEach(player::sendMessage));
    }

    public void broadcastResetMessage() {
        String finalResetMessage = Utils.setPlaceholders(getResetMessage(), this);
        getPlayersToWarn().forEach(player -> Arrays.stream(finalResetMessage.split("/n")).forEach(player::sendMessage));
    }
    ...
    public CataMineType getType() {
        if (type != null) {
            return type;
        }

        if (region instanceof ConvexPolyhedralRegion) {
            return CataMineType.CONVEXPOLYHEDRAL;
        } else if (region instanceof PolyhedralRegion) {
            return CataMineType.POLYHEDRAL;
        } else if (region instanceof Polygonal2DRegion) {
            return CataMineType.POLYGONAL2D;
        } else if (region instanceof CylinderRegion) {
            return CataMineType.CYLINDER;
        } else if (region instanceof EllipsoidRegion) {
            return CataMineType.ELLIPSOID;
        } else if (region instanceof CuboidRegion) {
            return CataMineType.CUBOID;
        } else {
            return CataMineType.NONE;
        }
    }
